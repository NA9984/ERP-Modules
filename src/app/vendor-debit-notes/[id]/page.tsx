...
const QrCodeIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-12 h-12 text-slate-400">
    ...
    <path strokeLinecap="round" strokeLinejoin="round" d="M6.75 6.75h.75v.75h-.75v-.75ZM6.75 16.5h.75v.75h-.75v-.75ZM16.5 6.75h.75v.75h-.75v-.75ZM13.5 13.5h.75v.75h-.75v-.75ZM13.5 16.5h.75v.75h-.75v-.75ZM16.5 13.5h.75v.75h-.75v-.75ZM16.5 16.5h.008v.008H16.5V16.5Z" />
    <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 19.5h.75v.75h-.75v-.75ZM16.5 19.5h.75v.75h-.75v-.75ZM19.5 13.5h.75v.75h-.75v-.75ZM19.5 16.5h.75v.75h-.75v-.75ZM19.5 19.5h.75v.75h-.75v-.75ZM16.5 16.5h.008v.008H16.5V16.5Z" />
  </svg>
);

interface VendorDebitNoteHeader {
  id: string;
  number: string;
  description: string;
...
export default function VendorDebitNoteDetailPage() {
  const params = useParams();
  const router = useRouter();
  const debitNoteId = params.id as string;

  const [loading, setLoading] = useState(true);
  const [header, setHeader] = useState<VendorDebitNoteHeader | null>(null);
  const [items, setItems] = useState<any[]>([]);
  
  const [summary, setSummary] = useState<DebitNoteSummary>({
    subTotal: "0.00",
    discAmt: "0.00",
    beforeTax: "0.00",
    taxAmount: "0.00",
  ...
  async function loadHeader() {
    try {
      const response = await fetch(`${API_BASE_URL}/vendor-debit-note/${debitNoteId}`);
      if (response.ok) {
  ...
  async function loadItems() {
    try {
      const response = await fetch(`${API_BASE_URL}/vendor-debit-note-detail/by-vendor-debit-note/${debitNoteId}`);
      if (response.ok) {
        const data = await response.json();
        setItems(data.items || []);
      }
  ...
  async function loadSummary() {
    try {
      const [subTotalRes, discAmtRes, beforeTaxRes, taxAmountRes, totalRes] = await Promise.all([
        fetch(`${API_BASE_URL}/vendor-debit-note/${debitNoteId}/summary-sub-total`),
        fetch(`${API_BASE_URL}/vendor-debit-note/${debitNoteId}/summary-disc-amt`),
        fetch(`${API_BASE_URL}/vendor-debit-note/${debitNoteId}/summary-before-tax`),
        fetch(`${API_BASE_URL}/vendor-debit-note/${debitNoteId}/summary-tax-amount`),
        fetch(`${API_BASE_URL}/vendor-debit-note/${debitNoteId}/summary-total`)
      ]);
      ...
      <div className="min-h-screen erp-page/50 p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto font-sans antialiased">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 erp-card p-5 rounded-2xl border border-slate-200/80 shadow-sm">
          <div className="space-y-1">
            <div className="flex flex-wrap items-center gap-2.5">
              <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Vendor Debit Note Details</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          ...
          <div className="erp-card rounded-2xl border border-slate-200/80 p-5 flex items-center gap-4 shadow-sm">
            ...
            <div className="space-y-0.5 min-w-0">
              ...
              <span className="text-sm font-semibold text-slate-880 font-mono block truncate">{header?.number || "No Record Data"}</span>
            </div>
          </div>

          {/* OVERVIEW */}
          <div className="erp-card rounded-2xl border border-slate-200/80 p-5 shadow-sm space-y-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-slate-100 pb-2">Debit Note Overview</h3>
              ...
              <div className="grid grid-cols-2 gap-2 pt-1 border-t border-slate-50">
                <div>
                  ...
                  <span className="text-xs font-semibold text-slate-700">
                    {header?.debitNoteDate ? new Date(header.debitNoteDate).toLocaleDateString(undefined, { dateStyle: 'medium' }) : "—"}
                  </span>
                </div>
                <div>
                  <span className="block text-[10px] font-medium text-slate-400">Maturity Date</span>
                  <span className="text-xs font-semibold text-rose-600">
                    {header?.debitNoteDueDate ? new Date(header.debitNoteDueDate).toLocaleDateString(undefined, { dateStyle: 'medium' }) : "—"}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="erp-card border border-slate-200/80 rounded-2xl shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between erp-page/50">
            <div className="space-y-0.5">
              <h3 className="font-bold text-slate-800 tracking-tight text-base">Adjustment Cost Allocation</h3>
              <p className="text-xs text-slate-400">Breakdown of adjusted product rates, taxes, and dynamic totals</p>
            </div>
                ...
                {items.length === 0 && (
                  <tr>
                    <td colSpan={8} className="p-16 text-center text-slate-400 font-medium">
                      No matching line items registered to this debit note.
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div className="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-2xl shadow-md shadow-emerald-600/10 p-5 text-white sm:col-span-2 lg:col-span-1 flex flex-col justify-between space-y-2">
            <div className="text-[10px] font-bold text-emerald-100 uppercase tracking-wider">Total Adjustment Value</div>
